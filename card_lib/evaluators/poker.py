...
def generate_minimal_filler(existing_hand):
    def is_low_straight_candidate(hand):
        ...
        ranks.sort()
        if (max(ranks) == 14 and (max(ranks[:-1]) == 5 or max(ranks[:-1]) == 4 or max(ranks[:-1]) == 3)) or (max(ranks) == 6 and (min(ranks) == 4 or min(ranks) == 3 or min(ranks) == 2)) or (max(ranks) == 5 and min(ranks) == 2) or (max(ranks) == 5 and min(ranks) == 3) or (max(ranks) == 4 and min(ranks) == 2):
            return True
        return False
